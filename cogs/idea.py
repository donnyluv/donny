...
class Idea(commands.Cog):
    ...
    def _resolve_colour(self, guild: disnake.Guild) -> disnake.Colour:
    ...
    def format_rating_label(self, rating: int) -> str:
        ...
        return mapping.get(rating, f"{rating} звезд")
...
class IdeaMenuView(disnake.ui.View):
    ...
    def _build_buttons(self) -> List[disnake.ui.Button]:
        ...
        back_button = disnake.ui.Button(
            ...
            row=2,
            disabled=self.mode is IdeaViewMode.SUMMARY,
        )
        back_button.callback = self.on_back
        buttons.append(back_button)

        if self.mode in (IdeaViewMode.VIEW, IdeaViewMode.ALL):
            ...
            has_next = self.page < self.total_pages() - 1 and bool(self._current_ideas)
        ...
        elif self.mode is IdeaViewMode.PROPOSE:
            ...
            create_button = disnake.ui.Button(
                ...
                style=disnake.ButtonStyle.secondary,
                row=2,
                disabled=not self.idea_text.strip(),
            )
    ...
    async def on_edit(self, interaction: disnake.MessageInteraction) -> None:
        await interaction.response.send_message(
            "Отправьте в чат текст вашей идеи:", ephemeral=True
        )
        ...
        try:
            ...
            await interaction.followup.send(
                "Время ожидания истекло. Повторите попытку.", ephemeral=True
            )
            return
        ...
        if truncated:
            ...
            await interaction.followup.send("Текст идеи обновлен.", ephemeral=True)
    ...
    async def on_create(self, interaction: disnake.MessageInteraction) -> None:
        ...
        if member is None:
            await interaction.response.send_message(
                "Не удалось определить участника сервера.", ephemeral=True
            )
            return
        ...
        if not content:
            await interaction.response.send_message(
                "Пожалуйста, сначала введите текст предложения.", ephemeral=True
            )
            return
        ...
        if channel is None:
            await interaction.response.send_message(
                "Канал для идей не настроен. Обратитесь к администрации.",
                ephemeral=True,
            )
            return
        ...
        if record is None:
            await interaction.followup.send(
                "Не удалось создать предложение. Попробуйте позже.", ephemeral=True
            )
            return
        ...
        await interaction.followup.send(
            f"Предложение № {record.id} успешно создано.", ephemeral=True
        )
...
class IdeaListSelect(disnake.ui.Select):
    def __init__(self, view: IdeaMenuView) -> None:
        ...
        options: List[disnake.SelectOption] = [
            disnake.SelectOption(label=placeholder, value="prompt", default=True)
        ]
    ...
    async def callback(self, interaction: disnake.MessageInteraction) -> None:
        ...
        if idea is None or idea.guild_id != idea_view.guild.id:
            await interaction.response.send_message(
                "Не удалось найти выбранное предложение.", ephemeral=True
            )
            return
...
class IdeaMessageView(disnake.ui.View):
    def __init__(self, cog: Idea, idea: IdeaRecord) -> None:
        ...
        self.cog = cog
        self.idea = idea
        self.add_item(IdeaRatingSelect(self))
        self.add_item(IdeaAdminSelect(self))


class IdeaRatingSelect(disnake.ui.Select):
    def __init__(self, view: IdeaMessageView) -> None:
        self._idea_view = view
        ...
        if view.idea.status is not IdeaStatus.PENDING:
            self.disabled = True
    ...
    async def callback(self, interaction: disnake.MessageInteraction) -> None:
        ...
        if value.startswith("set_"):
            if idea.status is not IdeaStatus.PENDING:
                await interaction.response.send_message(
                    "Оценивание для этого предложения недоступно.", ephemeral=True
                )
                return
            ...
            try:
                ...
                await interaction.response.send_message(
                    "Некорректное значение оценки.", ephemeral=True
                )
                return
            ...
            if rating is None:
                await interaction.response.send_message(
                    f"Вы еще не оценивали предложение № {idea.id}.", ephemeral=True
                )
            else:
                rating_label = idea_view.cog.format_rating_label(rating)
                await interaction.response.send_message(
            ...
            if current_rating is None:
                await interaction.response.send_message(
                    f"У вас нет оценки для предложения № {idea.id}.", ephemeral=True
                )
                return
...
class IdeaAdminSelect(disnake.ui.Select):
    ...
    async def callback(self, interaction: disnake.MessageInteraction) -> None:
        ...
        if not idea_view.cog.is_idea_admin(interaction.user):
            await interaction.response.send_message(
                "У вас нет прав для управления предложениями.", ephemeral=True
            )
            return
        ...
        if value == "approve":
            if idea.status is not IdeaStatus.PENDING:
                await interaction.response.send_message(
                    "Это предложение уже рассмотрено.", ephemeral=True
                )
                return
            await interaction.response.defer(ephemeral=True)
            ...
            if refreshed is None:
                await interaction.followup.send(
                    "Не удалось найти предложение.", ephemeral=True
                )
                return
            await idea_view.cog.approve_idea(refreshed, guild, interaction.user)
            await interaction.followup.send(
                f"Вы одобрили предложение № {idea.id}.", ephemeral=True
            )
        elif value == "reject":
            if idea.status is not IdeaStatus.PENDING:
                await interaction.response.send_message(
                ...
                return
            modal = IdeaRejectModal(idea_view.cog, idea.id)
            ...
            await interaction.followup.send(
                f"Предложение № {idea.id} удалено.", ephemeral=True
            )
...
class IdeaRejectModal(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        if not reason:
            await interaction.response.send_message(
                "Укажите причину отклонения.", ephemeral=True
            )
            return
        ...
        if idea is None:
            await interaction.response.send_message(
                "Предложение не найдено.", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"Вы отклонили предложение № {idea.id}.", ephemeral=True
        )
