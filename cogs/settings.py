...
SYSTEM_COLOUR_PRESETS: Sequence[dict[str, object]] = [
    ...
    },
    {
        "label": "Бирюзовый",
        "value": 0x00B0F4,
        "description": "Холодный бирюзовый цвет для свежего оформления.",
        "emoji": "💙",
    },
    {
        "label": "Фиолетовый",
    ...
    def update_components(self) -> None:
        ...
        options: List[disnake.SelectOption] = [
            disnake.SelectOption(
                ...
                value="default",
                description="Использовать стандартный цвет Discord.",
                emoji="💠",
                default=current_raw is None,
            )
        ]
        ...
        for preset in SYSTEM_COLOUR_PRESETS:
            ...
            options.append(
                disnake.SelectOption(
                    ...
                    default=current_raw == value,
                )
            )

        self.select_menu.options = options
        self.select_menu.placeholder = self._build_placeholder(current_raw)

    def _build_placeholder(self, current_raw: int | None) -> str:
        label = self.cog.format_system_colour_label(current_raw)
        return f"Текущий цвет: {label}"
