...
class Settings(commands.Cog):
    ...
    BUTTON_APPEARANCE: dict[
        ...
        "clear": {"label": None, "emoji": "<:clears:1435483487321718855>"},
    ...
    def build_settings_embed(self, guild: disnake.Guild) -> disnake.Embed:
        embed = disnake.Embed(
            title="<:settings:1434448066274000917> Настройки сервера:",
    ...
    def build_auto_roles_embed(self, guild: disnake.Guild) -> disnake.Embed:
        ...
        embed = disnake.Embed(
            title="<:spisok:1435481990672679034> Автоматические роли",
    ...
    def build_idea_settings_embed(self, guild: disnake.Guild) -> disnake.Embed:
        ...
        embed = disnake.Embed(
            title="<:settings:1434448066274000917> Настройки предложений",
            description="\n".join(description_lines),
            colour=self.get_system_colour(guild.id),
        )
    ...
    def build_success_embed(self, guild: disnake.Guild) -> disnake.Embed:
        return disnake.Embed(
            title="<:odobreno:1434909104794501243> Настройки успешно сохранены",
            colour=self.get_system_colour(guild.id),
        )
    ...
    def build_system_colour_embed(self, guild: disnake.Guild) -> disnake.Embed:
        ...
        description_lines = [
            "*Выберите основной цвет, который будет использоваться во всех сообщениях от имени бота.*",
            "*Ваш выбор немедленно применяется ко всем новым системным сообщениям.*",
            "",
            f"*Текущий цвет:* `{self.format_system_colour_label(current_raw)}`",
        ]

        embed = disnake.Embed(
            title="<:colorsik:1435482873929928877> Цвет системных сообщений",
            description="\n".join(description_lines),
            colour=self.get_system_colour(guild.id),
        )
        ...
        if self.mode == "channel":
            ...
            options: List[disnake.SelectOption] = [
                ...
                disnake.SelectOption(
                    label="Не указано",
                    value="clear",
                    emoji="<:otkaz:1434909101242060902>",
    ...
    def __init__(self, view: IdeaSettingsView) -> None:
        options = [
            ...
            disnake.SelectOption(
                label="Выбрать канал",
                value="channel",
                emoji="<:channels:1434448044576870470>",
            ),
            disnake.SelectOption(
                label="Роли администрации",
                value="roles",
                emoji="<:administrator:1434913080344707122>",
            ),
        ]
        super().__init__(
